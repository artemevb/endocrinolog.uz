...
export default function Services() {
    const services = [
        ...
        {
            title: "Индивидуальный вдумчивый подход к каждому пациенту",
            icon: stars
        },
        {
            title: "Высокая квалификация, ответственность и профессионализм",
            icon: plus
        }
    ];
            ...
            <div className="grid grid-cols-1 mdx:grid-cols-2 xl:grid-cols-4 gap-[12px] mdx:gap-[16px] xl:gap-[20px]">
                {services.map((service, index) => (
                    <div key={index} className=" p-[24px] rounded-xl flex flex-col items-center border-[#EFEFEF] border " style={{ boxShadow: "0px 0px 50px 0px rgba(58, 71, 91, 0.05)" }}>
                        ...
                        <h3 className={`max-w-[177px] text-[18px] mdx:text-[22px] xl:text-[25px] font-semibold text-center pb-[20px] ${index === 0 ? 'max-w-[177px]' : 'max-w-[250px]'}`}>{service.title}</h3>
                    </div>
                ))}
            </div>
        </div>
